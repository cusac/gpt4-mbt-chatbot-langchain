...
def youtube_dl_log(d):
    ...
    if d['status'] == 'downloading':
        if 'downloaded_bytes' in d and 'total_bytes_estimate' in d and d['total_bytes_estimate'] > 0:
            print("Downloading video: " +
                  str(round(float(d['downloaded_bytes'])/float(d['total_bytes_estimate'])*100, 1))+"%")
        else:
            print("Downloading video: progress information not available")